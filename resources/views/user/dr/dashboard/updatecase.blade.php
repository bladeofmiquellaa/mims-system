...
@section('content')
    ...
    <form action="/user/doctor/updatingcase/{{$case['id']}}" method="post" enctype="multipart/form-data">
        <div class="case container">
            ...
            <div class="case-main" name="{{$access_token['token_body']}}">
                <div class="case-input">
                    ...
                    <div>
                        <label for="organs">عضو  بدن مرتبط</label>
                    ...
                    <div>
                        ...
                        <select class="category" name="category" >
                            ...
                            @if(isset($case['category_name']))
                                <option >{{$case['category_name']}}</option>
                            @endif
                    ...
                    <div>
                        ...
                        <select class="sub-category"  name="subcategory" >
                            @if(isset($case['subcategory_name']))
                                <option >{{$case['subcategory_name']}}</option>
                            @endif
                                <option>انتخاب سابکتگوری</option>
                                @foreach($subcategories as $subcategory)
                                    @if($subcategory['category_id']==$case['category_id'])
                                        @if($subcategory['id']==$case['subcategory_id'])
                    ...
                    <div>
                        ...
                        <textarea name="findings" >{{$case['findings']}}</textarea>
                    </div>
                    <div>
                        <label>تشخیص</label>
                        <textarea name="diagnosis">{{$case['diagnosis']}}</textarea>
                    </div>
                    <div>
                        <label>وضعیت</label>
                        <select name="state" >
                            @if($case['state']=="approved")
                                <option value="approved">انتشار پرونده</option>
                                <option value="not approved">عدم تایید</option>
                                <option value="dr_draft">ذخیره پیشفرض</option>
                            @elseif($case['state']=="not approved")
                                <option value="not approved">عدم تایید</option>
                                <option value="approved">انتشار پرونده</option>
                                <option value="dr_draft">ذخیره پیشفرض</option>
                            @else
                        ...
                        <div class="current-box">
                            @foreach($photos as $photo)
                                <div name="{{$photo['name']}}" id="{{$photo['id']}}" class=current>
                                    ...
                                    <img src="http://127.0.0.1:8000/case_images/{{$photo['name']}}" alt=""  name="{{$photo['name']}}">
